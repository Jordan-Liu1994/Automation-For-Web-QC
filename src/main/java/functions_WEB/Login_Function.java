...
public class Login_Function {
	...
	public void ClickLoginOptionButton() throws FailedLoginException {
		WebElement loginOptionButton = base_Driver.getDriver().findElement(By.id("header_login"));
		String loginOptionButton_Text = loginOptionButton.getText();
		String fail = "Login option button failed";

		if (loginOptionButton.isEnabled()) {
			loginOptionButton.click();
			create_Report.getExtentTest().info(loginOptionButton_Text + " is enabled & clicked");
		} else {
